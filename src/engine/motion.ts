...
/** Courtyard Harbor RS in the map art points ~11 o'clock. Add this so 0° faces north. */
export const MAP_RS_HEADING = 40
export const MAP_SCALE_FULL = 2.08
export const MAP_SCALE_COMPACT = 2.35

export function mapRsHeading(angle: number, offset = MAP_RS_HEADING) {
  return angle + offset
}

/** Pan the courtyard so map percent (x, y) sits on the viewport aim. */
export function cameraFrame(x: number, y: number, scale: number, aimX = 50, aimY = 56) {
  return {
    width: scale * 100,
    height: scale * 100,
    left: aimX - x * scale,
